...
function switchView(viewName) {
    ...
    document.getElementById('main-nav').style.display = viewName === 'home' ? 'flex' : 'none';
}
...
function startQuiz(categoryIndex) {
    ...
    currentQuestions = [...currentCategory.questions].sort(() => Math.random() - 0.5);
...
function handleAnswer(selectedBtn, selectedOption, q) {
    ...
    if (isCorrect) {
        ...
        buttons.forEach(b => {
            ...
            if (textSpan && textSpan.innerText.includes(q.answer)) {
                b.classList.add('correct');
            }
        });
        
        const isTF = q.options.length === 2 && 
                     q.options.some(o => typeof o === 'string' && o.toLowerCase().includes('true')) && 
                     q.options.some(o => typeof o === 'string' && o.toLowerCase().includes('false'));
                     
        const isAnswerFalse = typeof q.answer === 'string' && q.answer.toLowerCase().includes('false');
...
function updateStats() {
    elements.statRemaining.textContent = currentQuestions.length - currentQuestionIndex - (elements.btnNext.classList.contains('disabled') ? 0 : 1);
